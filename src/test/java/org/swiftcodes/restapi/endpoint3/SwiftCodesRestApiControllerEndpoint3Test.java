...
public class SwiftCodesRestApiControllerEndpoint3Test {
    ...
    void shouldAddSwiftCode() throws Exception {
        ...
        when(databaseManager.checkIfSwiftCodeAlreadyExists(poland.swiftCode.getSwiftCode())).thenReturn(false);
        when(databaseManager.saveCountry(any(Country.class))).thenReturn(poland.country);
        when(databaseManager.saveBank(any(Bank.class))).thenReturn(poland.bank);
        when(databaseManager.saveSwiftCode(any(SwiftCode.class))).thenReturn(poland.swiftCode);

        mockMvc.perform(post("/v1/swift-codes")
                 .contentType(MediaType.APPLICATION_JSON)
                 .content(objectMapper.writeValueAsString(poland.dto)))
                .andExpect(MockMvcResultMatchers.status().isCreated())
                .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Swift code successfully added!"));
    }


    @Test
    void shouldReturnSwiftCodeAlreadyExists() throws Exception {

        FakeRecord poland = createFakeRecord("Warszawa", "NBP", "PL", "Polska", true, "QWERTYUIXXX", 7777, 7777);

        when(databaseManager.checkIfSwiftCodeAlreadyExists(poland.swiftCode.getSwiftCode())).thenReturn(true);
        when(databaseManager.saveCountry(any(Country.class))).thenReturn(poland.country);
        when(databaseManager.saveBank(any(Bank.class))).thenReturn(poland.bank);
        when(databaseManager.saveSwiftCode(any(SwiftCode.class))).thenReturn(poland.swiftCode);
        ...
        mockMvc.perform(post("/v1/swift-codes")
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Swift code already exists!"));
    }
    ...
    void shouldReturnInternalServerError() throws Exception {
        ...
        when(databaseManager.saveCountry(any(Country.class))).thenReturn(poland.country);
        when(databaseManager.saveBank(any(Bank.class))).thenReturn(poland.bank);
        when(databaseManager.saveSwiftCode(any(SwiftCode.class))).thenReturn(null);

        mockMvc.perform(post("/v1/swift-codes")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(poland.dto)))
                .andExpect(MockMvcResultMatchers.status().isInternalServerError())
                .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Error while adding Swift code!"));
    }
